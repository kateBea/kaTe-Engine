...
#if !defined(IMGUI_IMPL_OPENGL_LOADER_CUSTO)
...
// Functions
bool ImGui_ImplOpenGL3_Init(const char *glslVersion) {
    ...
    */
    ...
    if (glslVersion == nullptr)
        glslVersion = fmt::format("#version {}{}0", KT_OPENGL_VERSION_MAJOR, KT_OPENGL_VERSION_MINOR).c_str();

    IM_ASSERT((int) strlen(glslVersion) + 2 < IM_ARRAYSIZE(bd->GlslVersionString));
    strcpy(bd->GlslVersionString, glslVersion);
