...
namespace kaTe {
   ...
   auto ImGuiLayer::RecordImGuiCommandBuffers(UInt32_T imageIndex) -> void {
       ...
       std::array<VkClearValue, 1> clearValues{}; // Only one clear value for the color attachment
       ...
       ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(), m_ImGuiCommandBuffers[0]);
   ...
   auto ImGuiLayer::CreateImGuiFrameBuffers() -> void {
       m_ImGuiCommandBuffers.resize(VulkanContext::GetSwapChain()->GetImageCount());
